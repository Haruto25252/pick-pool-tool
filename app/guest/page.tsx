  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-sm">
        ...
        <p className="text-gray-400 text-sm text-center mb-6">{t('guest.subtitle')}</p>
        <div className="grid gap-2 mb-4">
          {userList.length === 0 && <p className="text-gray-500 text-center py-4">{t('guest.noUsers')}</p>}
          {userList.map(u => (
            <button key={u.id} onClick={() => router.push(`/user/${u.username}`)}
              ...
              <span className="text-gray-400 text-sm">{t('guest.viewArrow')}</span>
            </button>
          ))}
        </div>
